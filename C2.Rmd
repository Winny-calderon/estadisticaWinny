...
names(co2_emi)[names(co2_emi) == 'Carbon dioxide emissions from consumption of energy'] <- 'CarbonEmissions'
```

Verificamos la falla:
```{r}
head(co2_emi)
```
...
Crear las columnas:
```{r}
co2_emi$emission1=str_split(string = co2_emi$CarbonEmissions, pattern = 'Mt', simplify = T)[,1]

co2_emi$emission2=str_split(string = co2_emi$CarbonEmissions, pattern = ' ', simplify = T)[,2]
```
...
tail(co2_emi)
```
OJO CON ESTO!! NO ME SALE :´()::
```{r}
co2_emi$emissions_2=replace(co2_emi$emissions_2, co2_emi$emissions_2=="million",10**6)
co2_emi$emissions_2=replace(co2_emi$emissions_2, co2_emi$emissions_2=="billion",10**9)
co2_emi$emissions_2=replace(co2_emi$emissions_2, co2_emi$emissions_2=="",10**0)
head(co2_emi)
```
...
democracy[2]
names(democracy)
democracy$`Changes fromlast year `=NULL
head(democracy)
democracy$Country=str_split(string = names(democracy),
          pattern = '>>', simplify = T) [,1]
```
